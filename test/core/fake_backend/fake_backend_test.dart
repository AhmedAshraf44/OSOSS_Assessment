  ...
  group('FakeBackend.submitCount', () {
    test('accepts a submission when no product version has moved', () async {
      final result = await backend.submitCount(
        ...
        request: {
          'clientSessionId': 'local-session-uuid',
          'storeId': 1,
    ...
    test('replaying the same idempotency key returns the cached result '
    ...
    test(
      'two different idempotency keys get two different server sessions',
      () async {
        final payload = [
          {'productId': 101, 'countedQuantity': 20, 'expectedVersion': 5},
        ];
    ...
    test('detects a version conflict reproducing the assessment\'s documented '
        'example (client saw v5/qty20, a sale moves it to v6/qty19)', () async {
    ...
    test(
      'does not flag a conflict when expectedVersion matches current',
      () async {
